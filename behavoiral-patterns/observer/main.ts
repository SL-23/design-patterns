interface MySubject {
  attach(observer: Observer): void;
  detach(observer: Observer): void;
  notify(): void
}

class ConcreteSubject implements MySubject {
  public state: number = 0;
  private observers : Observer[] = [];
  public attach(observer: Observer): void {
    const isExist = this.observers.includes(observer)
    if(isExist) return console.log("subject: observer has been attached already.")

    console.log("subject: attached an observer.")
    this.observers.push(observer)
  }
  public detach(observer: Observer): void {
    const observerIndex = this.observers.indexOf(observer)
    if(observerIndex === -1) return console.log("subject: nonexist observer.")
    this.observers.splice(observerIndex, 1);
    console.log("subject: detached an observer.")
  }
  public notify(): void {
    console.log("subject: notifying observers...");
    for(const observer of this.observers) {
      observer.update(this)
    }
  }

  public someBusinessLogic(): void {
    console.log("subject: doing sth important.")
    this.state = Math.floor(Math.random() * (10+1))

    console.log(`subject: my state has changed to : ${this.state}`)
    this.notify()
  }
}


interface Observer {
  update(subject: MySubject): void;
}

class ConcreteObserverA implements Observer {
  public update(subject: MySubject): void {
    if(subject instanceof ConcreteSubject && subject.state < 3) {
      console.log("concrete observer A: reacted to the event.")
    }
  }
}

class ConcreteObserverB implements Observer {
  public update(subject: MySubject): void {
    if(subject instanceof ConcreteSubject && (subject.state === 0 || subject.state >=2))
     console.log("concrete observer B: reacted to the event.")
  }
}

const subject = new ConcreteSubject()

const observer1 = new ConcreteObserverA();
subject.attach(observer1)

const observer2 = new ConcreteObserverB();
subject.attach(observer2)

subject.someBusinessLogic();
subject.someBusinessLogic();

subject.detach(observer2)

subject.someBusinessLogic()